...
public final class DefaultExtractorInput implements ExtractorInput {
  ...
  private int readFromDataSource(byte[] target, int offset, int length, int bytesAlreadyRead,
    ...
    int bytesRead=0;
    try {
      bytesRead = dataSource.read(target, offset + bytesAlreadyRead, length - bytesAlreadyRead);
      ...
      Log.e("EXTRACTOR", "Array bound error:   offset: "+(offset+bytesAlreadyRead)  +" len: " + (length-bytesAlreadyRead) +" bytesRead: "+bytesRead );
    }
